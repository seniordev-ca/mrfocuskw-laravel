...
/*!***********************************************************!*\
  !*** ./resources/metronic/js/pages/layout/review-list.js ***!
  \***********************************************************/
 // Class definition
...
var KTDatatableHtmlTableDemo = function () {
  ...
  var demo = function demo() {
    var datatable = $('#kt_datatable_reviews').KTDatatable({
      data: {
        saveState: {
          cookie: false
        }
    ...
    $('#kt_datatable_reviews').on("click", ".delete", function (e) {
      e.preventDefault();
      var id = $(this).data('id');
      Swal.fire({
        ...
        if (result.value) {
          ...
          var sendUri = HOST_URL + "/layout/review-list/" + id;
          $.ajax({
            url: sendUri,
            method: 'DELETE',
    ...
    $('#kt_datatable_reviews').on("click", ".edit", function (e) {
      e.preventDefault();
      var id = $(this).data('id');
      $(".page-loader").addClass("bg-dark-o-50");
      $("#kt_body").addClass("page-loading");
      ...
      $.ajax({
        ...
        complete: function complete(xhr, status) {
          if (xhr.status === 200 && xhr.responseJSON.data) {
            ...
            $("#editModal [name=client_name]").val(data.client_name);
            $("#editModal [name=client_name_ar]").val(data.client_name_ar);
            $("#editModal [name=client_job]").val(data.client_job);
            $("#editModal [name=client_job_ar]").val(data.client_job_ar);
            $("#editModal [name=content]").val(data.content);
            $("#editModal [name=content_ar]").val(data.content_ar);
